...
describe('InnerJoinManyFixture', () => {
    ...
    it('AddLeftOnly', () => {
        _people.addOrUpdateValues(range(1, 10).pipe(map(i => new Person('Person' + i, i))));
    ...
    function assertDataIsCorrectlyFormed(allPeople: Person[], ...missingParents: string[]) {
        ...
        for (let grouping of grouped) {
            if (missingParents.length > 0 && missingParents.includes(grouping.key)) {
                return;
            }
            ...
            expect(children).toEqual(toArray(grouping));
        }
    }
